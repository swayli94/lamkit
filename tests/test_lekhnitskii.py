...
def test_theoretical_far_field_uniaxial_x():
    ...
    x = np.array([R])
    y = np.array([0.0])
    sxx, syy, txy = midplane_stresses_combined_load(
        sig_inf, 0.0, 0.0, load=0.0, angle_load_degree=0.0,
        hole_radius=1.0, thickness=1.0, compliance_matrix=S,
        x=x, y=y
    )
...
def test_isotropic_stress_concentration_uniaxial_x():
    ...
    sxx, syy, txy = midplane_stresses_combined_load(
        sig_inf, 0.0, 0.0, load=0.0, angle_load_degree=0.0,
        hole_radius=a, thickness=1.0, compliance_matrix=S, x=x, y=y
    )
