...
from lamkit.utils import evaluate_loaded_hole_plate
...
def evaluate_laminate_loaded_hole_field(
    ...
    results_by_plies : list[dict]
        Ply-surface results (length ``2 * n_ply``) from ``evaluate_loaded_hole_plate``.
    """
    ...
    results_by_plies, mid_plane_field = evaluate_loaded_hole_plate(
        laminate,
        hole_radius,
        load,
        thickness,
...
def envelope_fi_of_all_plies(
    ...
    """
    Through-thickness envelope of LaRC05 fields from ``evaluate_loaded_hole_plate``.
